...
beta_value = np.array([1000, 2000, 5000, 6000, 10000, 20000, 30000, 40000])


louvain_NMI_score = pd.DataFrame(index=Data_name, columns=beta_value)
louvain_ARI_score = pd.DataFrame(index=Data_name, columns=beta_value)
Kept_component = []
...
for d in range(len(Data_name)):
    ...
    for j in range(beta_value.shape[0]):
        beta = beta_value[j]
        print('\n++++++ SCARP++++++')
        ...
        louvain_NMI_score[beta][data_name] = SCARP_score['NMI']['louvain']
        louvain_ARI_score[beta][data_name] = SCARP_score['ARI']['louvain']
...
louvain_NMI_score1 = pd.DataFrame(scipy.stats.zscore(louvain_NMI_score, axis=1),
                                  index=Data_name, columns=beta_value)
...
plt.xlabel('beta')
plt.show()
plt.savefig('./Results/beta sensitivity louvain_NMI.svg', bbox_inches='tight')
louvain_NMI_score.to_csv('./Results/beta sensitivity louvain_NMI.txt', sep='\t')
...
plt.xlabel('beta')
plt.show()
plt.savefig('./Results/beta sensitivity louvain_ARI.svg', bbox_inches='tight')
louvain_ARI_score.to_csv('./Results/beta sensitivity louvain_ARI.txt', sep='\t')

np.savetxt('./Results/beta sensitivity Kept_component.txt', Kept_component)